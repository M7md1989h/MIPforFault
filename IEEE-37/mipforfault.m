clear all
    clc
load P
%    P%to get the measured voltage index (15)
PMU=16; % 16 PMU are installed
k = find(P);
%% Y matrix
% Initialize OpenDSS
...
DSSText.command='Compile (ieee37.dss)'; 
...
DSSSolution=DSSCircuit.Solution;
% Run your OpenDSS file 
DSSText.command='Compile (ieee37.dss)';%place the OpenDss file path here 
...
formatSpec='new fault.LLL phases=2 bus1 = %s r=0';
A1=DSSCircuit.AllBusNames{s+2}
 fault=sprintf(formatSpec,A1);
 DSSText.command=fault;
DSSText.command='solve';
DSSSolution.Solve;
...
%  A1=DSSCircuit.AllNodeNames{s+6}
formatSpec='new fault.LLL phases=3 bus1 = %s r=0';
A1=DSSCircuit.AllBusNames{s+2}
 fault=sprintf(formatSpec,A1);
 DSSText.command=fault;
DSSText.command='solve';
